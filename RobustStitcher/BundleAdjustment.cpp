...
namespace calib {
    ...
    */
	...
	struct BundleAdjustmentRayError_single1 {
		BundleAdjustmentRayError_single1(double x1, double y1, double x2, double y2, double focal2, double camera21, double camera22, double camera23)
			: x1(x1), y1(y1), x2(x2), y2(y2), focal2(focal2), camera21(camera21), camera22(camera22), camera23(camera23) {}
		...
		bool operator()(const T* const camera1,
			...
			T point2[3];
			T focal22 = T(focal2);
			T camera2121 = T(camera21);
			T camera2222 = T(camera22);
			T camera2323 = T(camera23);
			point1[0] = T(x1); point1[1] = T(y1); point1[2] = T(1.0f);
			point2[0] = T(x2); point2[1] = T(y2); point2[2] = T(1.0f);

			T camera2[4];
			camera2[0] = camera2121;
			camera2[1] = camera2222;
			camera2[2] = camera2323;
			camera2[3] = focal22;
		...
		// the client code.
		static ceres::CostFunction* Create(const double x1, const double y1,
			const double x2, const double y2, const double focal2, const double camera21,
			const double camera22, const double camera23) {
			return (new ceres::AutoDiffCostFunction<BundleAdjustmentRayError_single1, 3, 4>(
				new BundleAdjustmentRayError_single1(x1, y1, x2, y2, focal2, camera21, camera22, camera23)));
		}
...
int calib::BundleAdjustment::refine_BA(std::vector<int>& index, calib::Imagefeature& current_feature, std::vector<calib::Imagefeature>& features,
				...
				calib::CameraParams& current_para)     //参数里存放的是和拼接图像相连的图像组
{
	double cameraParam[72*4];    //隐患！！！！！！！！！！！！！！！！！！！！！！！！！
	cv::SVD svd2;
	for (size_t i = 0; i < cameras.size(); i++) {
		svd2(cameras[i].R, cv::SVD::FULL_UV);
		cv::Mat R = svd2.u * svd2.vt;
		R = R.t();
		if (determinant(R) < 0)
			R *= -1;
		double R_[9];
	...
	double current[4];
	cv::SVD svd1;
	svd1(current_para.R, cv::SVD::FULL_UV);
	cv::Mat R1 = svd1.u * svd1.vt;
	R1 = R1.t();
	if (determinant(R1) < 0)
		R1 *= -1;
	double R_[9];
	double Rvec_[3];
	R_[0] = R1.at<double>(0, 0); R_[1] = R1.at<double>(0, 1); R_[2] = R1.at<double>(0, 2);
	R_[3] = R1.at<double>(1, 0); R_[4] = R1.at<double>(1, 1); R_[5] = R1.at<double>(1, 2);
	R_[6] = R1.at<double>(2, 0); R_[7] = R1.at<double>(2, 1); R_[8] = R1.at<double>(2, 2);
	ceres::RotationMatrixToAngleAxis(R_, Rvec_);
	current[0] = Rvec_[0];
	current[1] = Rvec_[1];
	current[2] = Rvec_[2];
	current[3] = current_para.focal;

	size_t connectionNum = index.size();
	// Create residuals for each observation in the bundle adjustment problem. The
	// parameters for cameras and points are added automatically.
	ceres::Problem problem;
	for (size_t edge_idx = 0; edge_idx < connectionNum; edge_idx++) {       //对每对图像进行处理
		/*if (current_matchesInfo[edge_idx].confidence < 1.5)
			continue;*/
		if (current_matchesInfo[edge_idx].confidence < 2.9)
			continue;
		const calib::Imagefeature& features1 = current_feature;
		const calib::Imagefeature& features2 = features[edge_idx];
		const calib::Matchesinfo& matches_info = current_matchesInfo[edge_idx];      //第几个可信任的图像对
		double focal2 = cameraParam[index[edge_idx] * 4 + 3];
		for (size_t k = 0; k < matches_info.matches.size(); ++k) {     //对每对特征点进行处理
			if (!matches_info.inliers_mask[k])    //特征对不可信任
				continue;
			const cv::DMatch& m = matches_info.matches[k];
			cv::Point2f p1 = features1.keypt[m.queryIdx].pt;
			cv::Point2f p2 = features2.keypt[m.trainIdx].pt;
			...
			// image location and compares the reprojection against the observation.
			ceres::CostFunction* cost_function =
				calib::BundleAdjustmentRayError_single1::Create(p1.x - features1.imgsize.width / 2,
					p1.y - features1.imgsize.height / 2,
					p2.x - features2.imgsize.width / 2,
					p2.y - features2.imgsize.height / 2, focal2, cameraParam[index[edge_idx] * 4 + 0],
					cameraParam[index[edge_idx] * 4 + 1], cameraParam[index[edge_idx] * 4 + 2]);
			problem.AddResidualBlock(cost_function,
				NULL /* squared loss */,
				&current[0]);
		}
	...
	current_para.focal = current[3];
	double R_1[9];
	Rvec_[0] = current[0];
	Rvec_[1] = current[1];
	Rvec_[2] = current[2];
	ceres::AngleAxisToRotationMatrix(Rvec_, R_1);      //将旋转向量转换为旋转矩阵，用于最后的拼接
	...
	current_para.R = current_para.R.t();
